...
export class EventService {
  ...
  public isCanStartEvent(event: Event): boolean {
    const totalMembers: number = Event.getUsersCount(event);
    const minimalMembers: number = event.minTeamMembers * 2;
    return (
      this.isEventOwner(event) &&
      event.status === EventStatus.Published &&
  ...
  public isAlreadyInEvent(event: Event, userId: number): boolean {
    return (
      event.teams?.filter((t) => t.members?.filter((x) => x.id === userId).length > 0).length > 0
    );
  }
  ...
  public isCanOnModeration(event: Event): boolean {
    return Boolean(event?.id) && this.isEventOwner(event) && !event.approvalApplicationId;
  }
