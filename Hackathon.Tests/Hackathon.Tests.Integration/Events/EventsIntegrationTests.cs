...
namespace Hackathon.Tests.Integration.Events;

public class EventsIntegrationTests : BaseIntegrationTest
{
    public EventsIntegrationTests(TestWebApplicationFactory factory)
        : base(factory)
    {
    }
    ...
    public async Task Update_When_UserIsNotOwner()
    {
        ...
        Assert.Contains(EventsErrorMessages.NoRightsExecuteOperation, response.Error.Content);
    }
    ...
    [Fact(DisplayName = "Есть возможность создать событие с тегами")]
    public async Task Create_Should_Store_Tags()
    {
        //arrange
        var eventModel = TestFaker.GetEventModels(1, TestUser.Id).First();
        var createEventRequest = Mapper.Map<CreateEventRequest>(eventModel);
        ...
        Assert.NotNull(eventCreationResponse);
        Assert.NotNull(eventCreationResponse.Content);
        Assert.NotNull(eventCreationResponse.Content.Tags);
        
        eventCreationResponse.Content.Tags
            .Should()
            .BeEquivalentTo(eventModel.Tags);
    }
