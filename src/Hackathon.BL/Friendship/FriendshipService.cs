...
namespace Hackathon.BL.Friendship;
...
public class FriendshipService: IFriendshipService
{
    ...
    public Task<BaseCollection<Common.Models.Friend.Friendship>> GetOffersAsync(
    ...
    public async Task<Result> CreateOrAcceptOfferAsync(long proposerId, long userId)
    {
        ...
        {
            return Result.NotFound(FriendshipErrorMessages.CantCreateAcceptFriendshipOffersForYourProfile);
        }
        ...
        {
            return Result.NotFound(FriendshipErrorMessages.UsersAreAlreadyFriends);
        }
        ...
        if (offer.ProposerId == proposerId)
        {
            switch (offer.Status)
            {
                case FriendshipStatus.Pending:
                    return Result.NotValid(FriendshipErrorMessages.OfferOfFriendshipWasSentEarlier);
    ...
    public async Task<Result> RejectOfferAsync(long userId, long proposerId)
    {
        ...
        if (offer is null)
        {
            return Result.NotValid(FriendshipErrorMessages.NoOffersOfFriendshipHaveBeenMade);
        }

        if (offer.Status != FriendshipStatus.Pending)
        {
            return Result.NotValid(FriendshipErrorMessages.InappropriateStatus);
        }
    ...
    public async Task<Result> UnsubscribeAsync(long proposerId, long userId)
    {
        ...
        {
            return Result.NotValid(FriendshipErrorMessages.YouAreNotSubscribedToUser);
        }
    ...
    public async Task<Result> EndFriendship(long firstUserId, long secondUserId)
    {
        ...
        {
            return Result.NotValid(FriendshipErrorMessages.UsersAreNotFriends);
        }

        await _friendshipRepository.UpdateFriendship(offer.ProposerId, offer.UserId, new Common.Models.Friend.Friendship
        {
