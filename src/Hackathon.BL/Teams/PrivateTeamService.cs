...
public class PrivateTeamService: IPrivateTeamService
{
    ...
    public async Task<Result<long>> CreateJoinRequestAsync(TeamJoinRequestCreateParameters parameters)
    {
        ...
        if (user is null)
        {
            return Result<long>.NotFound(UserErrorMessages.UserDoesNotExists);
        }

        var team = await _teamRepository.GetAsync(parameters.TeamId);
        if (team is null)
        {
            return Result<long>.NotFound(TeamErrorMessages.TeamDoesNotExists);
        }
        ...
        {
            return Result<long>.NotValid(TeamErrorMessages.UserAlreadyIsTheTeamMember);
        }

        if (team.IsFull())
        {
            return Result<long>.NotValid(TeamErrorMessages.TeamIsFull);
        }
    ...
    public async Task<Result<BaseCollection<TeamJoinRequestModel>>> GetTeamSentJoinRequests(long teamId, long authorizedUserId, PaginationSort paginationSort)
    {
        ...
        if (team is null)
        {
            return Result<BaseCollection<TeamJoinRequestModel>>.NotFound(TeamErrorMessages.TeamDoesNotExists);
        }
    ...
    public async Task<Result> ApproveJoinRequest(long authorizedUserId, long requestId)
    {
        ...
        if (team is null)
        {
            return Result.NotFound(TeamErrorMessages.TeamDoesNotExists);
        }
